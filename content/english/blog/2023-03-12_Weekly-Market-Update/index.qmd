...
week <- get_week(Sys.Date()-1)
sp500_sectors_sql <- get_sql(paste0(here(), "/R/SNP500_Sectors.sql"))
...
sp500_sectors%>%
  ...
  select(ticker, sector, open, high, low, close, weekly_return) %>%
  arrange(ticker) %>%
  flextable::flextable() %>%
  set_header_labels(values = c("ticker" = "Ticker", "sector" = "Sector", "price_week" = "Week", "weekly_return" = "Weekly Return", "close" = "Close", "high" = "High", 
                               "low" = "Low", "open" = "Open")) %>%
  colformat_double(j=7, big.mark=",", suffix = "%", digits = 2,
  na_str = "N/A")%>%
  colformat_double(j=c(3,4,5,6), big.mark=",", digits = 1,
  na_str = "N/A")%>%
  theme_zebra()
