...
Treasuries%>%
  ...
  select(Date, `2s3mos`, `10s2s`, `10s3mos`)%>%
  pivot_longer(cols=c(`2s3mos`, `10s2s`, `10s3mos`))%>%
  filter(!is.na(value))%>%
ggplot(aes(x= Date))+
 geom_line(aes(y=value, group=name, color=factor(name, levels=c("2s3mos", "10s2s", "10s3mos"))), size=1)+
  labs(x= "Date",
       y="Percentage",
       title = "Yield curves")+
  theme_custom()+
