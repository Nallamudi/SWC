...
data_date <- "2023-01-01"
 ...
 daily_metrics <- swc %>% 
  tbl(in_schema("price","daily_metrics")) %>% 
  filter(ticker == "RUT") %>%
  filter(date > data_date) %>%
  collect()

all_equity_tickers <- swc %>% 
  tbl(in_schema("setup","tickers")) %>% 
  filter(ib_type %in% c("STK","IDX")) %>%
  collect()
 
ib_failed_tickers <- swc %>% 
  tbl(in_schema("price","processed_tickers_hist")) %>% 
    left_join("tickers", by=c("ticker" = "ticker")) %>%
...
DOW Jones lost \~2.5% in March so far. It continues to under perform other indices this year.
